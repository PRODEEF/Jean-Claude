...
export type WeekGridProps = {
  /** Les 7 jours de la semaine, lundi en tête. */
  days: Date[];
 ...
 *
 * Sept colonnes et une échelle d'heures : c'est la forme commune au Calendrier
 * iOS, à Google Calendar et à Fantastical (§4.2). La journée entière est
 * sortie de l'échelle, en bandeau sous les jours — la placer à minuit
 * laisserait croire à un événement de début de nuit.
 */
export function WeekGrid({ days, events, onOpenEvent, onCreateAt }: WeekGridProps) {
    ...
    <View className="border-border flex-1 overflow-hidden rounded-xl border">
      ...
      <ScrollView
        ...
        onLayout={() => {
          ...
          scrollRef.current?.scrollTo({ y: INITIAL_HOUR * HOUR_HEIGHT, animated: false });
        }}
      >
            ...
            <View key={column.day.toISOString()} className="border-border flex-1 border-l">
              ...
              <Pressable
                ...
                onPress={(gesture) =>
                  onCreateAt(
                    column.day,
                    Math.floor(gesture.nativeEvent.locationY / HOUR_HEIGHT) * 60,
                  )
                }
              />
